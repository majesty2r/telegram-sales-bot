...
async def send_main_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    text = t(context, "welcome", shop_name=SHOP_NAME)
...
async def add_to_cart(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = t(context, "added_to_cart", name=product["name"])
...
def build_cart_text(context: ContextTypes.DEFAULT_TYPE, cart: dict) -> str:
    ...
    for pid, qty in cart.items():
        ...
        if product:
            lines.append(
                t(context, "cart_line", name=product["name"], qty=qty, price=format_price(product))
            )
...
async def show_order_summary(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    lines.append(t(context, "cart_total", total=format_total(cart)))
    lines.append(t(context, "summary_payment", method=context.user_data.get("payment_method", "N/A")))
    if context.user_data.get("payment_proof") == "photo":
        ...
        lines.append(t(context, "summary_proof_text", proof=context.user_data["payment_proof"]))
